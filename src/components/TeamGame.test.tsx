/** @file Integration tests: one team's game, and the animator's reset. */
...
describe('Game', () => {
  ...
  it('plays from home to the victory, with a wrong answer and a wrong code', async () => {
    ...
    render(<TeamGame config={config} teamIndex={0} onChangeTeam={() => {}} />)
  ...
  it('shows the entrance message before the first step and starts the clock on the right answer', async () => {
    const user = userEvent.setup()
    render(<TeamGame config={{ ...config, entrance: { message: 'Qui suis-je ?', answer: { kind: 'letters', value: 'Bouh' } } }} teamIndex={0} onChangeTeam={() => {}} />)
    await user.click(screen.getByRole('button', { name: 'Commencer' }))
  ...
  it('shows the great hall behind the game, not on the home screen', async () => {
    const user = userEvent.setup()
    const { container } = render(<TeamGame config={config} teamIndex={0} onChangeTeam={() => {}} />)
  ...
  it('shows the restaurant front behind the entrance message, not the great hall', async () => {
    ...
    const { container } = render(<TeamGame config={{ ...config, entrance }} teamIndex={0} onChangeTeam={() => {}} />)
    await user.click(screen.getByRole('button', { name: 'Commencer' }))
